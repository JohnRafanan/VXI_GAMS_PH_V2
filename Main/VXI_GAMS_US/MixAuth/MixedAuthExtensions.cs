...
namespace VXI_GAMS_US
{
    ...
    public static class MixedAuthExtensions
    {
        ...
        {
            var routeData = new RouteData {RouteHandler = new MvcRouteHandler()};
            ...
            switch (action)
            {
                ...
                case Action.Logoff:
                    ...
                    break;
                default:
                    throw new NotSupportedException($"unknonw action value '{action}'.");
            }
            return routeData;
        }
        ...
        {
            var userId = handler.Context.Session[userIdKey] as string;
        ...
        {
            return htmlHelper.BeginForm("Login", "Windows", routeValues, FormMethod.Post, htmlAttributes);
        }
    }
}