...
{
    public class ManufacturerApiController : ApiController, IApiController<Manufacturer>
    {
        #region Implementation of IApiController<in Manufacturer>
        ...
        public async Task<IHttpActionResult> Get()
        {
            try
            {
                ...
                {
                    return Ok(await db.Manufacturers.Where(x => x.IsActive).ToListAsync());
                }
            }
            catch (Exception e)
            {
        ...
        public async Task<IHttpActionResult> Get(Guid id)
        {
            try
            {
                using (var db = new DataContext())
                {
                    var entity = await db.Manufacturers.Where(x => x.Id == id).FirstOrDefaultAsync();
                    if (entity == null)
        ...
        public async Task<IHttpActionResult> Post(Manufacturer value)
        {
            try
            {
                using (var db = new DataContext())
                {
                    var entity = await db.Manufacturers.FirstOrDefaultAsync(x => x.Id == value.Id);
                    if (entity == null)
                    {
                        value.CreatedBy = User.Identity.GetUserName();
                        value.Code = DataHelper.AlphaSpaceOnly(value.Code);
                        db.Manufacturers.Add(value);
                    }
                    else
                    {
        ...
        public async Task<IHttpActionResult> Put(Guid id, Manufacturer value)
        {
            try
            {
                using (var db = new DataContext())
                {
                    var entity = await db.Manufacturers.FindAsync(id);
                    if (entity == null)
                        throw new Exception("Record is either deleted or not existing.");
                    entity.Code = DataHelper.AlphaSpaceOnly(value.Code);
                    entity.DateUpdated = DateTime.Now;
        ...
        public async Task<IHttpActionResult> Delete(Guid id)
        {
            try
            {
                using (var db = new DataContext())
                {
                    var entity = await db.Manufacturers.FirstOrDefaultAsync(x => x.Id == id);
                    if (entity == null)
                        throw new Exception("Record is either deleted or not existing");
