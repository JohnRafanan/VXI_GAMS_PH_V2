...
{
    public class RetrieveApiController : ApiController
    {
        [HttpPost]
        public async Task<IHttpActionResult> BulkUpload()
        {
            try
            {
                ...
                {
                    await db.Database
                        .ExecuteSqlCommandAsync($"DELETE FROM dbo.RemoveAssignedAssetEmployeeUploads WHERE CreatedBy = '{user}'");
                }
                //check if there is an uploaded excel file
                if (HttpContext.Current.Request.Files.Count > 0)
                {
                    var file = HttpContext.Current.Request.Files["File"];
                    var bulkData = new List<RemoveAssignedAssetEmployeeUploads>();
                    if (file != null && file.ContentLength > 0)
                    {
                        var phTimeZone = TimeZoneInfo.FindSystemTimeZoneById("Singapore Standard Time");
                        var phDateTime = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, phTimeZone);
                        var tmpFileName = $"{user}-RETRIEVE-{phDateTime:yyyyddMMHHmmss}{phDateTime.TimeOfDay.Milliseconds}.xlsx";
                        var path = HttpContext.Current.Server.MapPath($"~/uploads/files/RemoveAssignedAsset/{phDateTime:MM-dd-yyyy}/{user}/");
                        ...
                        if (File.Exists(path))
                        {
                            ...
                            try
                            {
                                ...
                                using (var wb = new XLWorkbook(path))
                                {
                                    ...
                                    for (var i = 2; i < total; i++)//default do not change
                                    {
                                        ...
                                        var trackingNo = row.Cell(2).GetValue<string>();
                                        var ticketNo = row.Cell(3).GetValue<string>();
                                        ...
                                        bulkData.Add(new RemoveAssignedAssetEmployeeUploads
                                        {
                                            CreatedBy = user,
                                            Code = code,
                            ...
                            if (bulkData.Any())
                            {
                                IEnumerable<IEnumerable<RemoveAssignedAssetEmployeeUploads>> batches;
                                    ...
                                    .Select(chunks => Task.Run(async () =>
                                        {
                                            try
                                            {
                                                ...
                                                {
                                                    context.RemoveAssignedAssetEmployeeUploads.AddRange(chunks);
                                ...
                                {
                                    await db.Database
                                        .ExecuteSqlCommandAsync($"EXEC [dbo].[spRemoveAssignedAssetEmployeeUploads] @createdBy = '{user}'");
                                }
                            }
                        }
                    }
